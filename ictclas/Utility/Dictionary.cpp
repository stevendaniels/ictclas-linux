...
/*********************************************************************
 *
 *  Func Name  : LoadNewWords
 *
 *  Description: 加载用户自定义词典。自定义字典的格式为：每行一个词，每行以回车换行结束，不处理多余的空格。                 
 *              
 *
 *  Parameters : sFilename: the file name
 *
 ...
 *              1.create 2007-2-1
 *********************************************************************/
bool CDictionary::LoadNewWords(char *sFilename, int nHandle)
{
    FILE *fp;
    ...
    while (!feof(fp))
    {
        if (fgets(sLineBuffer, 256, fp) == 0)
            continue;
        nLen = strlen(sLineBuffer);
        assert(nLen > 0);
        if (nLen == 0)
            continue;
        sLineBuffer[nLen-1] = 0;
        AddItem(sLineBuffer, nHandle, 200);
    }
...
{ 
    ...
    int nPos,nFoundPos;
    PWORD_CHAIN pRet=NULL,pTemp,pNext;
...
int CDictionary::GetFrequency(char *sWord, int nHandle)
{
    ...
    if(FindInModifyTable(nPos,sWordFind,nHandle,&pFound) && (pFound != NULL))
    {
