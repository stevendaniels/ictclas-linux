...
bool IsAllNum(unsigned char *sString)
{
	...
	char sChar[3];
	sChar[2]=0;
	int firstNumPosCheck = 0;
	int secondNumPosCheck = 0;
	int thirdNumPosCheck = 0;
	int slashNum = 0;
	...
	if(i>=nLen)
	{
		if(secondNumPosCheck || firstNumPosCheck )
		{
			if(slashNum)
			{
				if(secondNumPosCheck)
				{
					return true;
				}
				else
	...
	if(i>=nLen)
	{
		if(secondNumPosCheck || firstNumPosCheck || thirdNumPosCheck)
		{
			if(slashNum > 10 )
			{
				if(secondNumPosCheck)
...
bool IsAllNum_0(unsigned char *sString)
{
